    ...
    <>
      <div className="mb-3 flex items-center gap-3">
        ...
        <h1 className="font-display text-[42px] font-semibold leading-none">
          {t.comingSoonPage.title} <span className="text-gold-gradient">{t.comingSoonPage.titleAccent}</span>
        </h1>
      </div>
      <p className="mb-8 text-sm text-gm-tx-2">{t.comingSoonPage.subtitle}</p>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-[18px]">
        {loading
          ...
          : movies.map((m) => (
              <MoviePosterCard key={m.id} movie={m} releaseMode cta={t.comingSoonPage.notify} />
            ))}
      </div>
    </>
  );
